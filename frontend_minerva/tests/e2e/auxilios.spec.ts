...
async function goToAuxilios(page: Page) {
  await page.goto("/auxilios");
  await expect(page.getByRole("heading", { name: /auxílios/i })).toBeVisible({ timeout: 10_000 });
}

async function openModal(page: Page) {
  await goToAuxilios(page);
  await page.getByRole("button", { name: /adicionar novo item/i }).click();
  await expect(page.getByRole("dialog")).toBeVisible();
}

test.describe("Auxílios — Página", () => {
  test("exibe título e ícone da página", async ({ page }) => {
    await goToAuxilios(page);
    await expect(page.getByRole("heading", { name: /auxílios/i })).toBeVisible();
  });

  test("exibe subtítulo da página", async ({ page }) => {
    await goToAuxilios(page);
    await expect(page.getByText(/gerenciamento de auxílios/i)).toBeVisible();
  });
...
test.describe("Auxílios — Modal", () => {
  test("abre o modal ao clicar em adicionar", async ({ page }) => {
    await openModal(page);
    await expect(page.getByRole("heading", { name: /novo auxílio/i })).toBeVisible();
  });
  ...
  test("exibe botão de submit com texto correto", async ({ page }) => {
    await openModal(page);
    await expect(page.getByRole("button", { name: /criar auxílio/i })).toBeVisible();
  });
});
