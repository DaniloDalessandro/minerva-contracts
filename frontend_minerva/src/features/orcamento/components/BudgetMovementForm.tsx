...
export function BudgetMovementForm({
  ...
  useEffect(() => {
    if (movement) {
      setFormData({
        source: typeof movement.source === 'object' ? (movement.source as Budget).id : movement.source as unknown as number,
        destination: typeof movement.destination === 'object' ? (movement.destination as Budget).id : movement.destination as unknown as number,
      ...
      <DialogContent className="sm:max-w-[600px] max-w-[90vw]">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            {errors.destination && <span className="text-sm text-destructive">{errors.destination}</span>}
          </div>
