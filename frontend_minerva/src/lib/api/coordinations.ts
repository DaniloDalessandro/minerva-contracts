...
const API_BASE_URL = API_ENDPOINTS.COORDINATION.LIST;
...
export async function createCoordination(data: { name: string, management_id: number }) {
  const res = await apiClient(API_ENDPOINTS.COORDINATION.CREATE, {
    method: "POST",
...
export async function updateCoordination(data: { id: number; name: string, management_id: number }) {
  const res = await apiClient(API_ENDPOINTS.COORDINATION.UPDATE(data.id), {
    method: "PUT",
...
export async function deleteCoordination(id: number) {
  const res = await apiClient(API_ENDPOINTS.COORDINATION.DELETE(id), {
    method: "PUT",
  });
