...
const API_BASE_URL = API_ENDPOINTS.MANAGEMENT.LIST;
...
export async function createManagement(data: { name: string, direction_id: number }) {
  const res = await apiClient(API_ENDPOINTS.MANAGEMENT.CREATE, {
    method: "POST",
...
export async function updateManagement(data: { id: number; name: string, direction_id: number }) {
  const res = await apiClient(API_ENDPOINTS.MANAGEMENT.UPDATE(data.id), {
    method: "PUT",
...
export async function deleteManagement(id: number) {
  const res = await apiClient(API_ENDPOINTS.MANAGEMENT.DELETE(id), {
    method: "PUT",
  });
