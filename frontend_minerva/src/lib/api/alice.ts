...
export type { ChatMessage, ChatResponse, QuickQuestionRequest, SessionStats, ConversationSession } from '@/types/entities/alice';
...
export async function submitFeedbackAPI(data: FeedbackRequest): Promise<{ success: boolean; message: string }> {
  const url = API_ENDPOINTS.ALICE.FEEDBACK
  const response = await apiClient(url, {
    method: 'POST',
    body: JSON.stringify(data),
  })
