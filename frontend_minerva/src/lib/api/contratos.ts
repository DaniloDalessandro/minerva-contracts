...
export type { Contract, ContractsResponse, CreateContractData };
...
export async function fetchContractsByBudgetLine(budgetLineId: number): Promise<Contract[]> {
  const params = new URLSearchParams({ budget_line: budgetLineId.toString(), page_size: "200" });
  const res = await fetchContractsAPI(params);
  return res.results;
}
